...
export type ClustersWatchSubscription = { clustersWatch: Array<{ id: string, spec: { name: string | null, syncEnabled: boolean, enabled: boolean, source: { kubeconfig: { context: string } | null } }, status: { source: { kubeconfig: { cluster: string, user: string, isPresent: boolean, isDefault: boolean } | null }, server: { uid: string | null }, conditions: Array<{ type: string, status: ConditionStatus, reason: string }> }, cache: { status: { lastSyncedAt: string | null, conditions: Array<{ type: string, status: ConditionStatus, reason: string }> }, stats: { exists: boolean, bytes: number } } }> };

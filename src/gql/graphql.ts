...
export type ClusterCachesWatchSubscription = { clusterCachesWatch: { type: ChangeType, cache: { id: string, clusterID: string, serverUid: string, status: { lastSyncedAt: string | null, conditions: Array<{ type: string, status: ConditionStatus, reason: string }> }, stats: { exists: boolean, bytes: number } } } };

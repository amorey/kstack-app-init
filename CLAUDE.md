...
**The sidecar's cluster backend is mid-rebuild** — everything below describes the wire contract, which is unchanged. What answers is the root `Cluster` surface — `cluster`, `clusters`, `clustersWatch`, `clusterScheduleWatch`, and the enable/sync/delete/`clusterConnectionRetry` mutations — plus the whole `ClusterCache` surface (the reads, `clusterCacheStatsWatch`, `clusterCacheHealthWatch`, and `clusterCacheClear`) and the `ClusterCachedCatalog` and `ClusterCachedResource` reads. **The `clusterCachedData*` queries and watches still panic**, and so does every `events(...)` field (`ListEvents`), so a query reaching one panics even though its parent resolves. A cache is created for each reachable cluster whose credentials can read `kube-system` (that UID is the identity a cache is named for), its catalog's pass discovers the kinds the cluster serves, and **a worker now mirrors each kind into that cache's local store** — but nothing reads those rows back out yet. The schema, the resolvers, and the whole webview are untouched and still compile; don't "fix" the frontend against the data that is still missing. → `sidecar/CLAUDE.md`.

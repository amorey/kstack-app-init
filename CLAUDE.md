...
**The sidecar's cluster backend is mid-rebuild** — everything below describes the wire contract, which is unchanged. What answers is the root `Cluster` surface — `cluster`, `clusters`, `clustersWatch`, `clusterScheduleWatch`, and the enable/sync/delete/`clusterConnectionRetry` mutations — plus the whole `ClusterCache` surface (the reads, `clusterCacheStatsWatch`, `clusterCacheHealthWatch`, and `clusterCacheClear`), the `ClusterCachedCatalog` and `ClusterCachedResource` reads, and the whole `clusterCachedData*` read path. Every `events(...)` field (`ListEvents`) still panics, so a query reaching one panics even though its parent resolves. A cache is created for each reachable cluster whose credentials can read `kube-system` (that UID is the identity a cache is named for), **but nothing fills one**: the seam between `ClusterCachedCatalog`, `ClusterCachedResource`, and the per-cache store is being redesigned from scratch, so no kind is discovered, no kind syncs, and every cached-data read answers empty. The schema, the resolvers, and the whole webview are untouched and still compile; don't "fix" the frontend against the data that is still missing. → `sidecar/CLAUDE.md`.

Cluster data streams as **Kubernetes-style delta watches** (an `Added` snapshot, one `Bookmark` closing it, then per-object `Added`/`Modified`/`Deleted`): `clustersWatch`, `clusterCachesWatch`, `clusterCachedCatalogsWatch` (all unscoped), and the cache-scoped `clusterCachedResourcesWatch` (opened only by whoever is looking at a cache). The sync verdict rides `clusterCacheHealthWatch`, which is **a gauge, not a delta watch** — current-on-subscribe, so no `Bookmark` closes it and none of the Bookmark discipline below applies. Each kind is its own stream; the webview keys each into a map and **joins client-side**: every child record carries an `owner { id kind }` ref, so caches join onto clusters and catalogs onto caches by `owner.id`; the verdict gauge keys on its own `cacheID` (a gauge is not an owned record). The `Bookmark` carries no entity (every change wrapper's entity field is nullable for it alone) and marks the snapshot complete — **never render an empty state before it lands**, or a populated table reads as empty for as long as the server takes to list it. **Detect it as `type === 'Bookmark'`, never as a missing entity**: a nested non-null field erroring nulls its parent, so a null entity also arrives on ordinary changes, and reading one as the snapshot boundary declares a still-loading collection complete. A change with no entity is dropped, not folded. → [ADR: delta-watch protocol](docs/adr/2026-08-09-delta-watch-protocol.md). The kube-context picker derives its list from `clustersWatch` (`src/lib/kube-config.tsx`).

**The sidecar never filters what it streams**, so a record awaiting deletion arrives like any other, carrying `deletionRequestedAt`; `Deleted` means its row is gone, and the mark that starts a teardown is an ordinary `Modified`. Deciding what to do with a dying record is the client's, and `ClustersProvider`'s fold makes it once — a tombstoned row folds as a removal, so no view has to remember. Render "Deleting…" there instead if that becomes the better answer; don't scatter the check into views.

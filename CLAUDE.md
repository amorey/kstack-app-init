...
**The sidecar's cluster backend is mid-rebuild** — everything below describes the wire contract, which is unchanged. What answers is the root `Cluster` surface — `cluster`, `clusters`, `clustersWatch`, `clusterScheduleWatch`, and the enable/sync/delete/`clusterConnectionRetry` mutations — plus the whole `ClusterCache` surface (the reads, `clusterCacheStatsWatch`, `clusterCacheHealthWatch`, and `clusterCacheClear`), the `ClusterCachedResource` reads, and the whole `clusterCachedData*` read path. Every `events(...)` field (`ListEvents`) still panics, so a query reaching one panics even though its parent resolves. A cache is created for each reachable cluster whose credentials can read `kube-system` (that UID is the identity a cache is named for), **but nothing fills one**: the seam between `ClusterCachedResource` and the per-cache store is being redesigned from scratch, so no kind is discovered, no kind syncs, and every cached-data read answers empty. The schema, the resolvers, and the whole webview are untouched and still compile; don't "fix" the frontend against the data that is still missing. → `sidecar/CLAUDE.md`.

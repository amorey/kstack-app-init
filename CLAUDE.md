...
- Path alias: **`@/*` → `src/*`**.
- App-wide concerns live in `src/lib/` (auth, error bus/boundary, sync-status, kube-config, ready-gate, `platform.ts` for sync `isMacOS()`/`isLinux()` UA detection); reusable UI in `src/components/widgets/`.

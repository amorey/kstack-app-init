...
**The sidecar's cluster backend is mid-rebuild** — everything below describes the wire contract, which is unchanged. What answers is the root `Cluster` surface — `cluster`, `clusters`, `clustersWatch`, `clusterScheduleWatch`, and the enable/sync/delete/`clusterConnectionRetry` mutations — plus the `ClusterCache` reads (`clusterCache`, `clusterCaches`, `clusterCachesWatch`, and the nested `Cluster.caches`) and the `ClusterCachedCatalog` reads. **Everything else still panics** — `Cluster.events`, the cache gauges, and the rest of the cache side — so a query reaching one panics even though its parent resolves. A cache is created for each reachable cluster whose credentials can read `kube-system` (that UID is the identity a cache is named for), and its catalog's pass discovers the kinds the cluster serves — but nothing syncs content yet, so the records below the catalog describe kinds no worker is mirroring. The schema, the resolvers, and the whole webview are untouched and still compile; don't "fix" the frontend against the data that is still missing. → `sidecar/CLAUDE.md`.

...
- Path alias: **`@/*` → `src/*`**.
- App-wide concerns live in `src/lib/` (auth, error bus/boundary, sync-status, kube-config, ready-gate, `platform.ts` for sync `isMacOS()`/`isLinux()` UA detection, `theme.tsx` for the color scheme); reusable UI in `src/components/widgets/`.
- **Theming**: `src/lib/theme.tsx` distinguishes two axes so a future skin feature slots in cleanly. (1) **Color scheme** — the user picks a `ColorSchemePreference` (`system`/`light`/`dark`, *when* the app is dark) that resolves to a concrete `ColorScheme` (`light`/`dark`; System follows `prefers-color-scheme`). It's class-based: the resolved scheme toggles a `.dark` class on `<html>` that Tailwind's `dark:` variant keys on (see `@custom-variant dark` in `index.css`; `@kubetail/ui` ships the `.dark` token overrides, and `index.css` paints the document `background` from the scheme's token so a new window's first frame is themed). The preference persists to `localStorage` under `kstack:color-scheme-preference`, resolves via `resolveColorScheme`, and is mirrored onto the root by `applyPreference`. To avoid a white flash when a window opens, an **inline script in `index.html`** applies the `.dark` class before the app bundle loads (it hand-mirrors the key + resolution in `theme.tsx`); `ThemeProvider` (in `__root.tsx`) then re-applies on mount, handles reactive changes, and syncs across windows via the `storage` event; components read it through `useColorScheme()` (`{ preference, setPreference }`). (2) **Theme (skin)** — the named palette within a resolved scheme (e.g. "github-light"): **not built yet**, but the word "theme" is reserved for it (`resolveColorScheme` is the seam, the resolved `ColorScheme` indexing `themes[scheme]`, applied later as a `data-theme` attribute; `kstack:theme*` keys are left free). The picker lives in the Settings dialog (`components/widgets/settings-dialog.tsx`, opened from the account menu via `openDialog('settings')`).

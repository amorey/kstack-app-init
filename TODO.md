...
- `src/lib/dashboard-nav.tsx` has pre-existing type errors (`pnpm exec tsc -b`) against the generated `ClusterDataKindsWatchSubscription` type: the reducer at `dashboard-nav.tsx:111` returns `Catalog | undefined` where a `SubscriptionHandler` requires `Catalog`, and line 130 reads `.cacheID`/`.kinds` off the subscription result, but the generated shape exposes those under `clusterDataKindsWatch` (`{ type, kind, cacheID }`), not at the top level. So the hand-written reducer is reaching for fields on the wrong node. Fix the accessors to match the generated `ClusterDataKindsWatchSubscription` shape (and give the reducer a non-`undefined` seed). Unrelated to the `Condition`/`ClusterCacheResourceStats` rename — `src/gql` is unchanged by that.

...
- **`opener:default`** in `src-tauri/capabilities/default.json` lets any script in the webview ask
  the host to open a URL or a file. Nothing in `src/` calls it. The tray's "Account" item opens a
  URL through `OpenerExt` in `src-tauri/src/tray/mod.rs`, which is Rust-side and needs no webview
  capability.
- **`chatStream`** is in the shipped GraphQL schema, and its resolver is
  `panic("not implemented: ChatStream")` (`sidecar/graph/schema.resolvers.go`). The panic is
...
`src-tauri/capabilities/default.json`. Leave `tauri_plugin_opener::init()` in `lib.rs` and the
dependency in `Cargo.toml` — the tray still uses them from Rust.
...
1. **Schema** (`sidecar/graph/schema.graphqls`): delete the `chatStream` field under the `# --- Chat ---`
   heading, and the `ChatInput` and `ChatChunk` types (around line 849). Delete the sentence in the
   `Subscription` type's comment that says "`chatStream` alone completes" — with it gone, every
   subscription is endless.
...
3. **Webview** (`src/routes/chat.tsx`): delete the `ChatStreamSubscription` document and the
   ...
   only exists to drive it. Keep the route and its layout. `send` appends the user's message and
   nothing answers; instead, disable the composer and render one line where the empty state is
   today: `Chat isn't available yet.` The point is that the route stays deep-linkable and the
   sidebar's mode switch keeps working.
...
Remove the *"Drop the two grants that have no use case yet"* item from
[`docs/TODO.md`](../TODO.md#security). Note in the root `CLAUDE.md`'s chat mention (if any) that the
chat route is a placeholder. When the assistant feature is built for real, re-add each grant scoped
to what it actually needs — and write the egress question down before the first token leaves.

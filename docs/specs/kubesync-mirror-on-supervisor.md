...
**Restart.** `RestartAll` cancels every handle it reads off the supervisor's snapshots. A clean exit
...
the supervisor rather than counted by hand, and `SinceAt` is stamped in `commitKind` rather than by
each writer.
...
- **Runs are counted for both supervisors.** `enterSweep`/`leaveSweep` become `enterRun`/`leaveRun`;
...
- **`close`** removes the session's subjects from both supervisors, cancels, waits, and releases the
...
- **`TestAColdListWaitingOnTheGateGivesUpWithItsRun`** becomes a test that a run queued behind the
  worker cap gives up with its session — `WithWorkers(1)` on the test supervisor, one run parked.
...
1. **The supervisor**: `Remove`/`Close` discard, the two doc sentences, the ADR.
2. **The mirror reconciler**: `kinds.go` split into the run and the goroutine, the handle, `Discard`,
   registered on a `mirrorSupervisor`; `kinds_test.go` moved onto the new harness. `session.go` still
   arms it through a worker at this step, so the old and new bodies can be diffed by their tests.
3. **The session**: `TrackKind`/`ForgetKind` as `Add`/`Remove`, the bridge waking both supervisors,

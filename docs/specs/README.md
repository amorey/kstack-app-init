...
**The cache read path, in order.** The goal is the dashboard nav and its tables, and the writer has
to come before the reader — `Objects` resolves a plural through `kind_catalog`, so the family
answers nothing until the table has rows. 1 and 2 are the path; 3 and 4 are cleanups behind it,
and neither blocks anything.

| # | Spec | Scope | Status | Delivers |
| --- | --- | --- | --- | --- |
| 1 | [Kind catalog sync](1-kind-catalog-sync.md) | sidecar | Planned | `kind_catalog` written by the sweep, live off the existing watch |

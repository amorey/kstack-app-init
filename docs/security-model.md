...
| The socket lives in a private per-user directory, and a directory left by an earlier run is adopted only once this user is proved to own it | `src-tauri/src/services/sidecar/service.rs` | **Enforced** — `runtime_dir_is_owner_only`, `runtime_dir_path_is_per_user_outside_xdg`, `ensure_private_dir_refuses_a_symlink_without_touching_its_target`. `$XDG_RUNTIME_DIR/kstack` when the session manager gives one, else `/tmp/kstack-<uid>` created 0700; the ownership check comes before the chmod, so another user's symlink is refused rather than tightened |
...
| A dispatched release version cannot reach a shell as source | `.github/workflows/release.yml` | **Held by review** — the input crosses into the config script as an environment value, and must match `major.minor.patch` with an optional dot-separated prerelease. Nothing exercises the rule; [TODO](TODO.md#security) |
...
| The host forwards only operations the app ships | — | **By decision, not built** — [no GraphQL operation allowlist](adr/2026-09-03-no-graphql-operation-allowlist.md); CSP reduces injection risk but does not restrict the authority of permitted bundled code. What the schema's breadth costs is S-9's other half, [TODO](TODO.md#security) |
| The app tells the user a newer release exists | — | **Not built** — no spec yet; [TODO](TODO.md#security). Nothing checks, so a user can sit on a version with a known bug indefinitely |
...
| Query cost, concurrency, fan-out and total-disk budgets | — | **Not built** — no spec yet; [TODO](TODO.md#security). The outer request-size and timeout bounds above are not budgets on the work an operation causes |
| Third-party actions pinned to SHAs, and a release gated on the checks for its own commit | — | **Not built** — no spec yet; [TODO](TODO.md#security). Actions float on tags (`dtolnay/rust-toolchain@master` on a branch), `release.yml` grants write permissions to every job, and audits run only per pull request |
| The macOS bundle grants only the hardened-runtime exceptions it needs | `src-tauri/entitlements.plist` | **Not built** — no spec yet; [TODO](TODO.md#security). Three exceptions ship whose necessity has never been tested against a signed build |

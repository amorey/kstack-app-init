...
- `state.rs` — `AppState` (sidecar, window_manager, shutdown), shared via `app.manage` / `app.state::<AppState>()`. `shutdown` is a `tokio_util::sync::CancellationToken` cancelled once on Quit (`lib.rs` `RunEvent::ExitRequested`, before the sidecar teardown); the app-lifetime background tasks select on it for clean shutdown — see below.
...
- `services/sidecar/` — sidecar process + IPC. (Native-host auth was removed in the auth/settings refactor; the frontend still drives its own auth flow.)

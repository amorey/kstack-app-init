...
- `commands.rs` — `#[tauri::command]` handlers the webview invokes (`graphql_query/subscribe/unsubscribe`, `ready`, `new_window`, `quit`). Keep thin — delegate to services. (`new_window`/`quit` back the Linux/Windows `AppMenu`; `quit` routes through `AppHandle::exit` for graceful shutdown.)

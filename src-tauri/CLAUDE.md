...
- `commands.rs` — `#[tauri::command]` handlers the webview invokes (`graphql_query/subscribe/unsubscribe`, `ready`, `new_window`, `quit`). Keep thin — delegate to services. (`new_window`/`quit` back the Linux/Windows `FileMenu`; `quit` routes through `AppHandle::exit` for graceful shutdown.)
...
- `app_menu.rs` — application menu bar construction. **macOS only** — it builds the global menu bar there; on Linux/Windows `build_app_menu` is a no-op (the native menu would render *inside* each window) and the webview's `FileMenu` replaces it, driving the same actions via the `new_window` / `quit` commands.
...
- `window_manager.rs`, `dock_menu.rs` (macOS), `error.rs`. **The webview's floating sidebar doubles as the window title bar**, so every window is chromeless: `build_window` gives macOS an Overlay title bar (native traffic lights kept, title text hidden) with the lights repositioned into the sidebar header via `traffic_light_position` (Tauri core — no plugin; requires Overlay + decorations left on), and makes Linux/Windows frameless (`decorations(false)`) so the webview draws its own controls. The statically-declared `"main"` window carries the macOS Overlay config in `tauri.conf.json`; `apply_main_window_chrome` (called in `setup`) drops decorations on that first window off macOS, since JSON can't be platform-conditional. The traffic-light inset arithmetic (`traffic_light_position`) is a pure, unit-tested free function; the pixel constants are kept in sync with `src/components/widgets/app-sidebar.tsx`.

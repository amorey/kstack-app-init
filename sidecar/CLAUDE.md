...
- **GraphQL diagnostics exclude request data.** The error presenter logs only operation type and Go error type (`TestErrorLogOmitsRequestData`); variables, inline literals, aliases, names and error messages can contain sensitive values. Other diagnostic paths can still carry sensitive remote error text (R-06 in the latest review). GraphQL and gRPC auth projections omit tokens. `TestAuthProjectionCarriesNoTokens` pins the GraphQL fields. OAuth credentials necessarily cross the network to the issuer.

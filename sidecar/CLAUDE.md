...
- `ClusterCachedDataEvent.type` is a plain `String!` (k8s doesn't constrain it) and timestamps are nullable `Time` autobound to **value** `time.Time` — which the delta-watch diff needs, since it compares frames with `==`. No resolver maps absence: gqlgen's `Time` marshaler emits `null` for the zero time, which is the wire's meaning of a timestamp the source object never carried.

...
- `internal/` — `ipc`, `atomicjson`, `logging`, `safe`, `sqlitemigrate`, `appdb`, `poke`, `kubeconfig`, `drain`, `lifecycle`, `workqueue`, `supervisor`, `testutil` (test-only, imported by no production code), plus the subsystems below.

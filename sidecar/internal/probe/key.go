...
// Key pairs a probe's registration name with the type it observes, declared once beside the
// probe rather than restated at every read. The pairing is checked where Get checks it — when a
// value lands, which in practice is the first test that reads one — and a key declared with the
// same name constant its probe registers under cannot get the name half wrong.
//
...
// From is this probe's observation out of snap, with the name and type already paired. It

...
func TestSyncedConditionMapsEveryState(t *testing.T) {
	tests := []struct {
		...
		{"watching is the only healthy state", kubesync.Status{State: kubesync.StateWatching}, ConditionTrue, ReasonWatching},
		{"errored surfaces the failure", kubesync.Status{State: kubesync.StateErrored, LastError: "boom"}, ConditionFalse, ReasonSyncFailed},
		{"stale is a fault, not a wait", kubesync.Status{State: kubesync.StateStale, Cause: kubesync.CauseWatchStalled}, ConditionFalse, ReasonStale},
		{"syncing is the catch-up state", kubesync.Status{State: kubesync.StateSyncing}, ConditionFalse, ReasonSyncing},
	}
...
func TestSyncEventNamesEachTransition(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			wantType:   beehive.EventNormal,
			wantReason: ReasonResyncComplete,

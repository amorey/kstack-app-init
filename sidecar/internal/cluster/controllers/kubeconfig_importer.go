...
func NewKubeconfigImporter(cfgSource KubeConfigSource, coreClient beehive.Client[domain.ClusterSpec, domain.ClusterStatus]) *KubeconfigImporter {
...
func (im *KubeconfigImporter) ClusterClient() beehive.Client[domain.ClusterSpec, domain.ClusterStatus] {
...
func (im *KubeconfigImporter) createCluster(ctx context.Context, contextName string) error {
	_, err := im.coreClient.Create(ctx, domain.KubeconfigName(contextName), domain.ClusterSpec{
		SyncEnabled: true,
		Enabled:     true,
		Source:      domain.ClusterSpecSource{Kubeconfig: &domain.ClusterSpecSourceKubeconfig{Context: contextName}},
	})
	return err
}

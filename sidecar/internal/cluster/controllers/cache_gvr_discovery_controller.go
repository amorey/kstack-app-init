...
type ClusterCacheGVRDiscoveryController struct {
	...
	stats map[beehive.ObjectID]domain.ClusterCacheGVRDiscoveryStats
}
...
func NewClusterCacheGVRDiscoveryController(rt *controllerRuntime) *ClusterCacheGVRDiscoveryController {
	return &ClusterCacheGVRDiscoveryController{
		...
		gvrSyncClient: beehive.NewClient[domain.ClusterCacheGVRSyncSpec, domain.ClusterCacheGVRSyncStatus](rt.bh, domain.ClusterCacheGVRSyncGroupKind),
		connMgr:       rt.connMgr,
		cacheManager:  rt.cacheManager,
		policies:      rt.policies(),
		newDiscovery:  newLiveDiscovery,
		stats:         make(map[beehive.ObjectID]domain.ClusterCacheGVRDiscoveryStats),
	}
}
...
func (c *ClusterCacheGVRDiscoveryController) Stats(objID beehive.ObjectID) (domain.ClusterCacheGVRDiscoveryStats, bool) {
	c.mu.Lock()
	defer c.mu.Unlock()
	st, ok := c.stats[objID]
	return st, ok
...
func (c *ClusterCacheGVRDiscoveryController) recordPass(objID beehive.ObjectID, resourceCount int) {
	c.mu.Lock()
	defer c.mu.Unlock()
	c.stats[objID] = domain.ClusterCacheGVRDiscoveryStats{
...
func (c *ClusterCacheGVRDiscoveryController) Reconcile(
	ctx context.Context,
	client beehive.ControllerClient[domain.ClusterCacheGVRDiscoveryStatus],
	obj *beehive.Object[domain.ClusterCacheGVRDiscoverySpec, domain.ClusterCacheGVRDiscoveryStatus],
	...
	if !obj.Spec.Enabled {
		...
		return beehive.Result{}, c.writeGate(ctx, client, obj, domain.ReasonPaused, "GVR discovery is paused")
	}
	...
	restCfg, _ := c.connMgr.Get(domain.ClusterID(clusterObjID))
	if restCfg == nil {
		...
		err := c.writeGate(ctx, client, obj, domain.ReasonNoConnection, "Waiting for a connection to the cluster")
	...
	if err != nil {
		...
		if condErr := c.writeGate(ctx, client, obj, domain.ReasonDiscoveryFailed, err.Error()); condErr != nil {
	...
	if held > 0 {
		...
		if condErr := c.writeGate(ctx, client, obj, domain.ReasonDiscoveryDraining,
	...
	cond := domain.LiveCondition(domain.ConditionDiscovered, domain.ConditionTrue, domain.ReasonDiscovered, "")
	if partial {
		cond = domain.LiveCondition(domain.ConditionDiscovered, domain.ConditionFalse, domain.ReasonDiscoveryPartial,
...
func (c *ClusterCacheGVRDiscoveryController) discover(restCfg *rest.Config) (resources []domain.ClusterCacheGVRSyncSpec, partial bool, err error) {
...
func syncableGVRs(lists []*metav1.APIResourceList) []domain.ClusterCacheGVRSyncSpec {
	total := 0
	...
	out := make([]domain.ClusterCacheGVRSyncSpec, 0, total)
	for _, list := range lists {
		if list == nil {
			continue
		}
		for _, r := range list.APIResources {
			...
			out = append(out, domain.ClusterCacheGVRSyncSpec{
...
func (c *ClusterCacheGVRDiscoveryController) syncChildren(
	...
	desired []domain.ClusterCacheGVRSyncSpec,
	prune bool,
) (existing []*beehive.Object[domain.ClusterCacheGVRSyncSpec, domain.ClusterCacheGVRSyncStatus], held int, err error) {
	wanted := make(map[string]domain.ClusterCacheGVRSyncSpec, len(desired))
	for _, spec := range desired {
		...
		spec.Enabled = true
		wanted[domain.ClusterCacheGVRSyncName(discoveryID, spec.APIVersion, spec.Resource)] = spec
	}
...
func (c *ClusterCacheGVRDiscoveryController) sweepOrphanedKinds(
	ctx context.Context,
	ref store.CacheRef,
	children []*beehive.Object[domain.ClusterCacheGVRSyncSpec, domain.ClusterCacheGVRSyncStatus],
) {
...
func (c *ClusterCacheGVRDiscoveryController) ensureEventsChild(
	...
	name := domain.ClusterCacheGVRSyncName(discoveryID, spec.APIVersion, spec.Resource)
...
func (c *ClusterCacheGVRDiscoveryController) writeGate(
	...
	cond := domain.LiveCondition(domain.ConditionDiscovered, domain.ConditionFalse, reason, message)

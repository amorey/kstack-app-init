...
type gvrDiscoveryFixture struct {
	...
	client    beehive.Client[domain.ClusterCacheGVRDiscoverySpec, domain.ClusterCacheGVRDiscoveryStatus]
	syncs     beehive.Client[domain.ClusterCacheGVRSyncSpec, domain.ClusterCacheGVRSyncStatus]
	api       *fakeDiscovery
	connMgr   *connections.Manager
	clusterCC beehive.ControllerClient[domain.ClusterStatus]
	// syncCC clears a child's drain finalizer the way the sync controller would — this
	// fixture runs a no-op in its place.
	syncCC beehive.ControllerClient[domain.ClusterCacheGVRSyncStatus]
	...
	discoveryCC beehive.ControllerClient[domain.ClusterCacheGVRDiscoveryStatus]
...
func newGVRDiscoveryFixture(t *testing.T) *gvrDiscoveryFixture {
	...
	clusterCC, err := beehive.Register(bh, domain.ClusterGroupKind, &NoopController[domain.ClusterSpec, domain.ClusterStatus]{})
	...
	require.NoError(t, err)
	discoveryCC, err := beehive.Register(bh, domain.ClusterCacheGVRDiscoveryGroupKind, ctrl)
	require.NoError(t, err)
	syncCC, err := beehive.Register(bh, domain.ClusterCacheGVRSyncGroupKind, &NoopController[domain.ClusterCacheGVRSyncSpec, domain.ClusterCacheGVRSyncStatus]{})
	require.NoError(t, err)
	...
	cacheObj, err := cacheClient.Create(ctx,
		domain.ClusterCacheName(domain.ClusterID(clusterObj.ID), testCacheUID),
		domain.ClusterCacheSpec{ServerUID: testCacheUID},
		beehive.WithOwner(clusterObj.ID))
	require.NoError(t, err)

	return &gvrDiscoveryFixture{
		ctrl:        ctrl,
		client:      beehive.NewClient[domain.ClusterCacheGVRDiscoverySpec, domain.ClusterCacheGVRDiscoveryStatus](bh, domain.ClusterCacheGVRDiscoveryGroupKind),
		syncs:       beehive.NewClient[domain.ClusterCacheGVRSyncSpec, domain.ClusterCacheGVRSyncStatus](bh, domain.ClusterCacheGVRSyncGroupKind),
		api:         api,
		connMgr:     connMgr,
		clusterCC:   clusterCC,
		syncCC:      syncCC,
		discoveryCC: discoveryCC,
		clusterID:   domain.ClusterID(clusterObj.ID),
		cacheID:     cacheObj.ID,
	}
}
...
func (f *gvrDiscoveryFixture) repoke(t *testing.T) {
	...
	require.NoError(t, f.clusterCC.UpdateStatus(context.Background(), beehive.ObjectID(f.clusterID), 1,
		domain.ClusterStatus{Server: domain.ClusterServer{Version: &version}}))
}

// createChild creates the discovery object the cache controller would, owned by the cache.
func (f *gvrDiscoveryFixture) createChild(t *testing.T, enabled bool) *beehive.Object[domain.ClusterCacheGVRDiscoverySpec, domain.ClusterCacheGVRDiscoveryStatus] {
	t.Helper()
	obj, err := f.client.Create(context.Background(),
		domain.ClusterCacheGVRDiscoveryName(f.cacheID),
		domain.ClusterCacheGVRDiscoverySpec{Enabled: enabled},
...
func (f *gvrDiscoveryFixture) syncChildren(t *testing.T, id beehive.ObjectID) []domain.ClusterCacheGVRSyncSpec {
	t.Helper()
	children, err := f.syncs.ListOwnedObjects(context.Background(), id)
	require.NoError(t, err)
	out := make([]domain.ClusterCacheGVRSyncSpec, 0, len(children))
...
func (f *gvrDiscoveryFixture) awaitSyncGVRs(t *testing.T, id beehive.ObjectID, want []string) []domain.ClusterCacheGVRSyncSpec {
	t.Helper()
	var last []domain.ClusterCacheGVRSyncSpec
...
func TestGVRDiscoveryReplacesADeletingChild(t *testing.T) {
	...
	f.repoke(t)
	cond := f.awaitDiscoveredReason(t, obj.ID, domain.ReasonDiscoveryDraining)
	assert.Equal(t, domain.ConditionFalse, cond.Status,
		"a kind whose child is still draining is not discovered-and-served")
	assert.Equal(t, []domain.ClusterCacheGVRSyncSpec{
	...
	f.repoke(t)
	f.awaitSyncGVRs(t, obj.ID, []string{"apps/v1/deployments", "v1/events", "v1/pods"})
	f.awaitDiscoveredReason(t, obj.ID, domain.ReasonDiscovered)
}
...
func TestGVRDiscoveryHandsADrainingWaitToBackoff(t *testing.T) {
	...
	require.NotZero(t, pods)
	require.NoError(t, f.syncs.Delete(ctx, pods))
	f.awaitDiscoveredReason(t, obj.ID, domain.ReasonDiscoveryDraining)
...
func (f *gvrDiscoveryFixture) awaitDiscoveredReason(t *testing.T, id beehive.ObjectID, want string) domain.Condition {
	t.Helper()
	return awaitConditionReason(t, f.client, id, domain.ConditionDiscovered, want)
}
...
func TestGVRDiscoveryCreatesOneChildPerSyncableGVR(t *testing.T) {
	...
	cond := f.awaitDiscoveredReason(t, obj.ID, domain.ReasonDiscovered)
	assert.Equal(t, domain.ConditionTrue, cond.Status)
...
func TestGVRDiscoveryPartialAnswerDoesNotPrune(t *testing.T) {
	...
	cond := f.awaitDiscoveredReason(t, obj.ID, domain.ReasonDiscoveryPartial)
	assert.Equal(t, domain.ConditionFalse, cond.Status)
...
func TestGVRDiscoveryFailureKeepsChildren(t *testing.T) {
	...
	cond := f.awaitDiscoveredReason(t, obj.ID, domain.ReasonDiscoveryFailed)
	assert.Equal(t, domain.ConditionFalse, cond.Status)
...
func TestGVRDiscoveryPauseRelaysToChildren(t *testing.T) {
	...
	_, err := f.client.Update(ctx, obj.ID, domain.ClusterCacheGVRDiscoverySpec{Enabled: false})
	require.NoError(t, err)

	cond := f.awaitDiscoveredReason(t, obj.ID, domain.ReasonPaused)
	assert.Equal(t, domain.ConditionFalse, cond.Status)
	require.Eventually(t, func() bool {
...
func TestGVRDiscoveryWaitsForConnection(t *testing.T) {
	...
	cond := f.awaitDiscoveredReason(t, obj.ID, domain.ReasonNoConnection)
	assert.Equal(t, domain.ConditionFalse, cond.Status)
	assert.Equal(t, []domain.ClusterCacheGVRSyncSpec{eventsSyncSpec(true)}, f.syncChildren(t, obj.ID),
...
// in-memory connection manager, so a cluster connecting changes nothing this object would

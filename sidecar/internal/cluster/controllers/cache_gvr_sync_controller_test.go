...
var testGVRSyncSpec = domain.ClusterCacheGVRSyncSpec{
	Enabled:    true,
	APIVersion: "apps/v1",
	Kind:       "Deployment",
...
type gvrSyncFixture struct {
	...
	client     beehive.Client[domain.ClusterCacheGVRSyncSpec, domain.ClusterCacheGVRSyncStatus]
...
func newGVRSyncFixture(t *testing.T) *gvrSyncFixture {
	...
	require.NoError(t, err)
	_, err = beehive.Register(bh, domain.ClusterCacheGVRDiscoveryGroupKind,
		&NoopController[domain.ClusterCacheGVRDiscoverySpec, domain.ClusterCacheGVRDiscoveryStatus]{})
	require.NoError(t, err)
	cc, err := beehive.Register(bh, domain.ClusterCacheGVRSyncGroupKind, ctrl)
	require.NoError(t, err)
	...
	discoveryClient := beehive.NewClient[domain.ClusterCacheGVRDiscoverySpec, domain.ClusterCacheGVRDiscoveryStatus](
		bh, domain.ClusterCacheGVRDiscoveryGroupKind)
	...
	discoveryObj, err := discoveryClient.Create(ctx,
		domain.ClusterCacheGVRDiscoveryName(cacheObj.ID),
	...
	return &gvrSyncFixture{
		ctrl:       ctrl,
		client:     beehive.NewClient[domain.ClusterCacheGVRSyncSpec, domain.ClusterCacheGVRSyncStatus](bh, domain.ClusterCacheGVRSyncGroupKind),
		factory:    factory,
		connMgr:    connMgr,
		cacheMgr:   cacheMgr,
		clusterCC:  clusterCC,
		...
		cacheRefID: domain.NewCacheRef(clusterObj.ID, cacheObj.ID),
	}
}
...
// connection manager, then write the parent Cluster's status — the wake that reaches a
...
func (f *gvrSyncFixture) createChild(t *testing.T, spec domain.ClusterCacheGVRSyncSpec) *beehive.Object[domain.ClusterCacheGVRSyncSpec, domain.ClusterCacheGVRSyncStatus] {
	t.Helper()
	obj, err := f.client.Create(context.Background(),
		domain.ClusterCacheGVRSyncName(f.discovery, spec.APIVersion, spec.Resource),
		spec,
...
func (f *gvrSyncFixture) awaitSyncedReason(t *testing.T, id beehive.ObjectID, want string) domain.Condition {
	t.Helper()
	return awaitConditionReason(t, f.client, id, domain.ConditionSynced, want)
}
...
func TestGVRSyncWorkersOfOneCacheShareOneListLimiter(t *testing.T) {
	...
	f.createChild(t, testGVRSyncSpec)
	f.factory.await(t)
	f.createChild(t, domain.ClusterCacheGVRSyncSpec{
...
func TestGVRSyncDisabledRunsNoWorker(t *testing.T) {
	...
	cond := f.awaitSyncedReason(t, obj.ID, domain.ReasonPaused)
...
// connection manager and writes the parent's status in one step, exactly as converge does.
...
func TestGVRSyncWaitsForConnection(t *testing.T) {
	...
	cond := f.awaitSyncedReason(t, obj.ID, domain.ReasonNoConnection)
	assert.Equal(t, domain.ConditionFalse, cond.Status)
...
func TestGVRSyncFoldsWorkerReports(t *testing.T) {
	...
	cond := f.awaitSyncedReason(t, obj.ID, domain.ReasonWatching)
	...
	assert.Equal(t, domain.ReasonSyncComplete, events[0].Reason)
...
func TestGVRSyncPokeIgnoresStoppedWorkers(t *testing.T) {
	...
	spec.Enabled = false
	obj := f.createChild(t, spec)
	f.awaitSyncedReason(t, obj.ID, domain.ReasonPaused)

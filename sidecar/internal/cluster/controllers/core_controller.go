...
type ProbeFunc func(ctx context.Context, cfg *rest.Config) (domain.ClusterServer, domain.ClusterPrincipal, error)
...
type ClusterCoreController struct {
	...
	ctrlClient  beehive.ControllerClient[domain.ClusterStatus]
	...
	sentinels  map[domain.ClusterID]*connSentinel
	...
	reconcileLocks map[domain.ClusterID]*sync.Mutex
	...
	probeMu  sync.Mutex
	probing  map[domain.ClusterID]bool
...
func NewClusterCoreController(
	...
	return &ClusterCoreController{
		cfgSource:      cfgSource,
		coreClient:     beehive.NewClient[domain.ClusterSpec, domain.ClusterStatus](rt.bh, domain.ClusterGroupKind),
		cacheClient:    beehive.NewClient[domain.ClusterCacheSpec, domain.ClusterCacheStatus](rt.bh, domain.ClusterCacheGroupKind),
		connMgr:        rt.connMgr,
		pokeSvc:        rt.pokeSvc,
		retryCh:        make(chan domain.ClusterID, retryBufferSize),
		probe:          probe,
		check:          check,
		sentinelWatch:  watchKubeSystem,
		sentinels:      make(map[domain.ClusterID]*connSentinel),
		reconcileLocks: make(map[domain.ClusterID]*sync.Mutex),
		probing:        make(map[domain.ClusterID]bool),
		probeHub:       probeHub,
...
func (c *ClusterCoreController) reconcileLock(id domain.ClusterID) *sync.Mutex {
	c.reconcileMu.Lock()
...
func (c *ClusterCoreController) setProbing(id domain.ClusterID, active bool) {
	c.probeMu.Lock()
...
// Probing field merged into Clusters().WatchSchedule.
func (c *ClusterCoreController) WatchProbe(ctx context.Context, id domain.ClusterID) <-chan bool {
	c.probeMu.Lock()
	cur := c.probing[id]
...
func (c *ClusterCoreController) recordAttempt(ctx context.Context, client beehive.ControllerClient[domain.ClusterStatus], id beehive.ObjectID, ok bool, reason, message string) {
	typ := beehive.EventWarning
	if ok {
		typ = beehive.EventNormal
	}
	err := client.AddEvent(ctx, id, beehive.EventSpec{
		Category: domain.ConnectionEventCategory,
		Type:     typ,
		Reason:   reason,
		Message:  domain.TruncateMessage(message),
	})
...
func (c *ClusterCoreController) SetControllerClient(cl beehive.ControllerClient[domain.ClusterStatus]) {
	c.ctrlClient = cl
}
...
func (c *ClusterCoreController) Reprobe(id domain.ClusterID) {
	select {
	case c.retryCh <- id:
	default:
	}
...
func (c *ClusterCoreController) reprobeOne(baseCtx context.Context, id domain.ClusterID) {
...
func (c *ClusterCoreController) reprobeObj(baseCtx context.Context, obj *beehive.Object[domain.ClusterSpec, domain.ClusterStatus]) {
...
func (c *ClusterCoreController) Reconcile(ctx context.Context, client beehive.ControllerClient[domain.ClusterStatus], obj *beehive.Object[domain.ClusterSpec, domain.ClusterStatus]) (beehive.Result, error) {
	mu := c.reconcileLock(domain.ClusterID(obj.ID))
	mu.Lock()
	defer mu.Unlock()
	...
	var loaded domain.ClusterStatus
	if obj.Status != nil {
		loaded = *obj.Status
	}
	working := domain.ClusterStatus{
...
func (c *ClusterCoreController) converge(ctx context.Context, client beehive.ControllerClient[domain.ClusterStatus], obj *beehive.Object[domain.ClusterSpec, domain.ClusterStatus], working *domain.ClusterStatus, conds *conditionSet) (time.Duration, error) {
	...
	if !connectionEligible(obj, present) {
		...
		conds.set(domain.LiveCondition(domain.ConditionConnected, domain.ConditionFalse, domain.ReasonInactive, ""))
		conds.set(domain.LiveCondition(domain.ConditionHealthy, domain.ConditionUnknown, domain.ReasonInactive, ""))
		return 0, nil
	}
	...
	if err != nil {
		c.teardownConnection(clusterID)
		c.recordAttempt(ctx, client, obj.ID, false, domain.ReasonResolveFailed, err.Error())
		return 0, c.observeConnectFailure(conds, domain.ReasonResolveFailed, err)
	}

	server, principal, err := c.probe(ctx, restCfg)
	if err != nil {
		c.teardownConnection(clusterID)
		c.recordAttempt(ctx, client, obj.ID, false, domain.ReasonProbeFailed, err.Error())
		return 0, c.observeConnectFailure(conds, domain.ReasonProbeFailed, err)
	}
	...
	// the connection manager to detect a credential rotation.
	fingerprint := connections.Fingerprint(restCfg, connections.ContextProxyURL(c.cfgSource.Get(), contextName))
	if c.connMgr != nil {
	...
	c.recordAttempt(ctx, client, obj.ID, true, domain.ReasonConnected, "")
	conds.set(domain.LiveCondition(domain.ConditionConnected, domain.ConditionTrue, domain.ReasonConnected, ""))
...
func (c *ClusterCoreController) observeKubeconfig(obj *beehive.Object[domain.ClusterSpec, domain.ClusterStatus], working *domain.ClusterStatus) bool {
	...
	if cfg := c.cfgSource.Get(); cfg != nil {
		if kctx, ok := cfg.Contexts[kc.Context]; ok && kctx != nil {
			working.Source.Kubeconfig = &domain.ClusterStatusSourceKubeconfig{
	...
	prev := domain.ClusterStatusSourceKubeconfig{}
...
func (c *ClusterCoreController) observeConnectFailure(conds *conditionSet, reason string, err error) error {
	conds.set(domain.LiveCondition(domain.ConditionConnected, domain.ConditionFalse, reason, err.Error()))
	conds.set(domain.LiveCondition(domain.ConditionHealthy, domain.ConditionUnknown, domain.ReasonNoConnection, ""))
	return err
}
...
func (c *ClusterCoreController) ensureClusterCache(ctx context.Context, clusterID domain.ClusterID, ownerID beehive.ObjectID, uid string) error {
	name := domain.ClusterCacheName(clusterID, uid)
	_, _, err := c.cacheClient.GetOrCreate(ctx, name, domain.ClusterCacheSpec{ServerUID: uid},
...
func (c *ClusterCoreController) pruneSupersededCaches(ctx context.Context, clusterID domain.ClusterID, ownerID beehive.ObjectID, activeUID string) {
...
func clusterIDFromObj(obj *beehive.Object[domain.ClusterSpec, domain.ClusterStatus]) domain.ClusterID {
	return domain.ClusterID(obj.ID)
}

// healthCondition maps one health-probe outcome onto the Healthy condition.
func healthCondition(phase HealthPhase, msg *string) domain.Condition {
	var message string
	if msg != nil {
		message = *msg
	}
	...
	case HealthPhaseHealthy:
		return domain.LiveCondition(domain.ConditionHealthy, domain.ConditionTrue, domain.ReasonReady, message)
	case HealthPhaseDegraded:
		return domain.LiveCondition(domain.ConditionHealthy, domain.ConditionFalse, domain.ReasonReadyzFailed, message)
	case HealthPhaseUnreachable:
		return domain.LiveCondition(domain.ConditionHealthy, domain.ConditionFalse, domain.ReasonUnreachable, message)
	default:
		return domain.LiveCondition(domain.ConditionHealthy, domain.ConditionUnknown, domain.ReasonNoConnection, message)
	}
}

// probeCluster dials the cluster for server version, kube-system UID, and the authenticated
// username.
func probeCluster(ctx context.Context, cfg *rest.Config) (domain.ClusterServer, domain.ClusterPrincipal, error) {
	...
	uid := string(ns.UID)
	return domain.ClusterServer{UID: &uid, Version: &ver.GitVersion},
		domain.ClusterPrincipal{Username: &ssr.Status.UserInfo.Username}, nil
}

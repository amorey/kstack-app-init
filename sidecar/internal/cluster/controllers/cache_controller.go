...
// read-side (Caches().WatchSyncHealth); see
...
func NewClusterCacheController(rt *controllerRuntime) *ClusterCacheController {
	return &ClusterCacheController{
		...
		gvrDiscoveryClient: beehive.NewClient[domain.ClusterCacheGVRDiscoverySpec, domain.ClusterCacheGVRDiscoveryStatus](rt.bh, domain.ClusterCacheGVRDiscoveryGroupKind),
		cacheManager:       rt.cacheManager,
	}
}

// Reconcile converges one ClusterCache object toward its parent Cluster's spec.
func (c *ClusterCacheController) Reconcile(ctx context.Context, client beehive.ControllerClient[domain.ClusterCacheStatus], obj *beehive.Object[domain.ClusterCacheSpec, domain.ClusterCacheStatus]) (beehive.Result, error) {
	...
	if obj.DeletionRequestedAt != nil {
		...
		if err := c.cacheManager.DeleteCacheFiles(ctx, domain.NewCacheRef(owner.ID, obj.ID)); err != nil {
			return beehive.Result{}, err
		}
	...
	active := domain.CacheIsActive(clusterObj, obj.Spec.ServerUID)
...
func (c *ClusterCacheController) clearCacheFilesFinalizer(ctx context.Context, client beehive.ControllerClient[domain.ClusterCacheStatus], obj *beehive.Object[domain.ClusterCacheSpec, domain.ClusterCacheStatus]) error {
...
func (c *ClusterCacheController) converge(
	ctx context.Context,
	client beehive.ControllerClient[domain.ClusterCacheStatus],
	cacheObjID beehive.ObjectID,
	active bool,
	clusterObj *beehive.Object[domain.ClusterSpec, domain.ClusterStatus],
	conds *conditionSet,
	prev []domain.Condition,
) time.Duration {
	...
	if syncSwitchedOff(clusterObj) && syncWasRunning(prev) {
		if !c.recordSyncStopped(ctx, client, cacheObjID) {
			...
			if old := domain.FindCondition(prev, domain.ConditionSynced); old != nil {
				conds.set(*old)
			}
	...
	reason := domain.ReasonSyncing
	if !enabled {
		reason = domain.ReasonPaused
	}
	conds.set(domain.LiveCondition(domain.ConditionSynced, domain.ConditionFalse, reason, ""))
...
func syncWasRunning(conds []domain.Condition) bool {
	cond := domain.FindCondition(conds, domain.ConditionSynced)
	return cond != nil && cond.Reason == domain.ReasonSyncing
}
...
func (c *ClusterCacheController) recordSyncStopped(ctx context.Context, client beehive.ControllerClient[domain.ClusterCacheStatus], cacheObjID beehive.ObjectID) bool {
	err := client.AddEvent(ctx, cacheObjID, beehive.EventSpec{
		Category: domain.SyncEventCategory,
		Type:     beehive.EventNormal,
		Reason:   domain.ReasonSyncStopped,
...
func (c *ClusterCacheController) ensureGVRDiscovery(
	...
	spec := domain.ClusterCacheGVRDiscoverySpec{Enabled: enabled}
	obj, created, err := c.gvrDiscoveryClient.GetOrCreate(ctx, domain.ClusterCacheGVRDiscoveryName(cacheObjID), spec,
...
func syncEligible(obj *beehive.Object[domain.ClusterSpec, domain.ClusterStatus]) bool {
...
func syncSwitchedOff(obj *beehive.Object[domain.ClusterSpec, domain.ClusterStatus]) bool {

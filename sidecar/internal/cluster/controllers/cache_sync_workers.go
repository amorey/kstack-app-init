...
func writeSyncGate[Status any](
	...
	return reportCondition(ctx, client, objID, generation,
		domain.LiveCondition(domain.ConditionSynced, domain.ConditionFalse, reason, message))
}

// syncedCondition maps one report onto the Synced condition; noun is needed only by Stale.
func syncedCondition(st kubesync.Status, noun string) domain.Condition {
	switch st.State {
	case kubesync.StateWatching:
		return domain.LiveCondition(domain.ConditionSynced, domain.ConditionTrue, domain.ReasonWatching, "")
	case kubesync.StateErrored:
		return domain.LiveCondition(domain.ConditionSynced, domain.ConditionFalse, domain.ReasonSyncFailed, st.LastError)
	case kubesync.StateStale:
		return domain.LiveCondition(domain.ConditionSynced, domain.ConditionFalse, domain.ReasonStale, staleMessage(st.Cause, noun))
	default:
		return domain.LiveCondition(domain.ConditionSynced, domain.ConditionFalse, domain.ReasonSyncing, "")
	}
}
...
func syncEvent(st kubesync.Status, prev kubesync.State, noun string) (beehive.EventType, string, string) {
	...
	case kubesync.StateWatching:
		if prev == kubesync.StateStale || prev == kubesync.StateErrored {
			return beehive.EventNormal, domain.ReasonResyncComplete,
				fmt.Sprintf("Watch recovered — streaming %s again", noun)
		}
		if st.ColdStart {
			return beehive.EventNormal, domain.ReasonSyncComplete, fmt.Sprintf(
				"Initial sync complete — cached %d %s in %s",
				st.SyncedItems, noun, roundSyncDuration(st.CaughtUpIn))
		}
		return beehive.EventNormal, domain.ReasonResyncComplete, resyncCompleteMessage(st, noun)
	case kubesync.StateErrored:
		return beehive.EventWarning, domain.ReasonSyncDegraded, st.LastError
	case kubesync.StateStale:
		return beehive.EventWarning, domain.ReasonSyncStale, staleMessage(st.Cause, noun)
	default: // StateSyncing
		if st.ColdStart {
			return beehive.EventNormal, domain.ReasonSyncStart, fmt.Sprintf("Starting initial %s sync", noun)
		}
		return beehive.EventNormal, domain.ReasonResyncStart, fmt.Sprintf(

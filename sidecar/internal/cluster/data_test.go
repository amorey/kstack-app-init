...
// recvKindFrame reads one delta off the ClusterDataKindsWatch stream, failing on
// timeout or an unexpected close.
func recvKindFrame(t *testing.T, ch <-chan domain.ClusterDataKindWatchFrame) domain.ClusterDataKindWatchFrame {
	t.Helper()
	return testutil.Recv(t, ch, "a ClusterDataKindWatchFrame")
}
...
func TestServiceClusterDataKindsWatch(t *testing.T) {
	...
	snap1 := recvKindFrame(t, ch)
	assert.Equal(t, domain.FrameAdded, snap1.Type)
	...
	snap2 := recvKindFrame(t, ch)
	assert.Equal(t, domain.FrameAdded, snap2.Type)
	...
	mod := recvKindFrame(t, ch)
	assert.Equal(t, domain.FrameModified, mod.Type)
	...
	add := recvKindFrame(t, ch)
	assert.Equal(t, domain.FrameAdded, add.Type)
	...
	del := recvKindFrame(t, ch)
	assert.Equal(t, domain.FrameDeleted, del.Type)
...
func TestServiceClusterDataKindsWatchCoalesces(t *testing.T) {
	...
	snap := recvKindFrame(t, ch)
	require.Equal(t, domain.FrameAdded, snap.Type)
...
func TestServiceClusterDataKindsWatchRebindsAfterCacheReplaced(t *testing.T) {
	...
	require.NoError(t, err)
	snap := recvKindFrame(t, ch)
	assert.Equal(t, domain.FrameAdded, snap.Type)
...
func recvEventFrame(t *testing.T, ch <-chan domain.ClusterDataEventWatchFrame) domain.ClusterDataEventWatchFrame {
	t.Helper()
...
func TestServiceClusterDataEventsWatchIgnoresObjectWrites(t *testing.T) {
	...
	ch, err := s.Data().WatchEvents(ctx, id, domain.ClusterCacheID(cacheID))
	require.NoError(t, err)
	require.Equal(t, "e1", recvEventFrame(t, ch).Event.UID) // snapshot
	bmEv := recvEventFrame(t, ch)
...
func TestServiceClusterDataObjectsWatchFiltersByKind(t *testing.T) {
	...
	ch, err := s.Data().WatchObjects(ctx, id, domain.ClusterCacheID(cacheID), "apps/v1", "deployments")
	require.NoError(t, err)
	require.Equal(t, "d1", recvObjectFrame(t, ch).Object.UID) // snapshot
	bm := recvObjectFrame(t, ch)
	requireBookmark(t, bm.Type, bm.Object)
...
func TestClusterDataKindsWatchWakesOnEventWrites(t *testing.T) {
	...
	for {
		ch2 := recvKindFrame(t, ch)
		if ch2.Kind.Kind != "Event" {
			continue
		}
		assert.Equal(t, domain.FrameModified, ch2.Type)
...
func TestClusterDataKindsWatchWakesOnKeyedWrite(t *testing.T) {
	...
	add := recvKindFrame(t, ch)
	if add.Type == domain.FrameBookmark {
		add = recvKindFrame(t, ch)
	}
	assert.Equal(t, domain.FrameAdded, add.Type)
...
func TestServiceClusterDataObjectsWatchSurvivesKindRemap(t *testing.T) {
	...
	require.NoError(t, err)
	require.Equal(t, "w1", recvObjectFrame(t, ch).Object.UID) // snapshot: the Widget object
	bm := recvObjectFrame(t, ch)
	requireBookmark(t, bm.Type, bm.Object)
	...
	got := map[string]domain.FrameType{}
	for len(got) < 2 {
		ev := recvObjectFrame(t, ch)
		got[ev.Object.UID] = ev.Type
	}
	assert.Equal(t, domain.FrameDeleted, got["w1"], "the old Kind's object must be removed")
	assert.Equal(t, domain.FrameAdded, got["g1"], "the replacement Kind's object must appear")
}

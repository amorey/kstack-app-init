...
func TestFreshnessFlushPersistsLastSynced(t *testing.T) {
	...
	// Drive time deterministically: a wall clock would collapse the open stamp
	// and the post-Notify stamp into the same millisecond on a fast machine,
	// making the strict ">" assertion below flake.
	...
	// A cache change (Notify) advances freshness. Advance the clock first so the
	// new stamp is unambiguously newer than the open stamp.
	clock.Store(2000)

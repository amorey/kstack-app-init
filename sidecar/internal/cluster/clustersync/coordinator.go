...
// viewHubCapacity bounds the per-subscriber backlog of merged-view snapshots.
// Each publish carries the complete current view, so a lagging subscriber's
// Chan feeder simply skips ahead to the newest snapshot — the ring only needs
// to absorb a small burst, never replay history.
const viewHubCapacity = 8
...
type Coordinator struct {
	...
	viewHub  *broadcast.Hub[[]ClusterView]
...
func NewCoordinator(cache *clustercache.Manager, watcher configSubscriber, registry *clusterregistry.Registry) *Coordinator {
	...
	return &Coordinator{
		...
		upstream: up,
		registry: registry,
		viewHub:  broadcast.New[[]ClusterView](viewHubCapacity),
...
func (c *Coordinator) reconcileLocked(ctx context.Context, cfg *api.Config) {
	...
	for name := range cfg.Contexts {
		...
		if err != nil {
			slog.Warn("clustersync: skip context (client config)", "context", name, "err", err)
			continue
		}
		wg.Add(1)
		go func(name string, restCfg *rest.Config) {
			...
			if err != nil {
				slog.Warn("clustersync: skip context (probe failed)", "context", name, "err", err)
				return
			}
			mu.Lock()
	...
	if err := c.registry.RecordSeenBatch(seen); err != nil { // one write per reconcile
		slog.Warn("clustersync: record clusters", "err", err)
	}
	...
	for _, r := range results {
		...
		if err != nil {
			...
			slog.Warn("clustersync: registry read failed during reconcile", "uuid", r.uid, "err", err)
			continue
		}
		if !ok || !rec.Enabled {
			continue
	...
	for uid, cl := range want {
		...
		case cl.fingerprint != existing.fingerprint:
			...
			slog.Info("clustersync: cluster config changed, restarting sync", "context", cl.Name, "uuid", uid)
			if err := c.closeCluster(ctx, uid, existing.Name); err != nil {
				...
				slog.Warn("clustersync: skipping sync restart, previous close failed", "uuid", uid, "err", err)
				continue
			}
			c.openCluster(ctx, cl)
		}
...
func (c *Coordinator) backfillOrphansOnce() {
	...
	if err != nil {
		slog.Warn("clustersync: scan cached clusters", "err", err)
		return
	}
	for _, uid := range uuids {
		_, ok, err := c.registry.Get(uid)
		if err != nil {
			slog.Warn("clustersync: registry read failed during backfill", "uuid", uid, "err", err)
			continue
		}
		if ok {
			continue
		...
		if _, err := c.registry.RecordSeen(uid, ""); err != nil { // "" → name unknown
			slog.Warn("clustersync: backfill cluster", "uuid", uid, "err", err)
		}
	}
}
...
func (c *Coordinator) openCluster(ctx context.Context, cl Cluster) {
	...
	if err != nil {
		slog.Error("clustersync: open cluster", "context", cl.Name, "uuid", cl.UUID, "err", err)
		c.upstream.RemoveConfig(cl.UUID)
		return
	}
	notify, unsub := cdb.Subscribe()
	go c.trackFreshness(cl.UUID, notify)
	slog.Info("clustersync: cluster cache opened", "context", cl.Name, "uuid", cl.UUID)
...
func (c *Coordinator) closeCluster(ctx context.Context, uuid, name string) error {
	...
	if err != nil {
		slog.Warn("clustersync: close cluster", "uuid", uuid, "err", err)
	}
	c.upstream.RemoveConfig(uuid)
	c.forgetOpen(uuid)
	slog.Info("clustersync: cluster cache closed", "context", name, "uuid", uuid)
	return err
}
...
func (c *Coordinator) flushFreshness() {
	...
	if err != nil {
		slog.Warn("clustersync: flush freshness", "err", err)
		return
	}
	if changed {
		c.publish()
...
// Subscribe streams the merged view on every change. The receiver sees only
// snapshots published after this call (the resolver emits the current snapshot
// itself); its Chan feeder coalesces past any lag to the newest view. The
// returned func closes the receiver. Implements Manager.
func (c *Coordinator) Subscribe() (<-chan []ClusterView, func()) {
	rx := c.viewHub.Receiver()
	return rx.Chan(), rx.Close
}

// publish recomputes and fans out the current merged view. Send never blocks;
// a lagging subscriber drops to the newest snapshot rather than stalling the
// publisher.
func (c *Coordinator) publish() { _ = c.viewHub.Sender().Send(c.buildViews()) }
...
func (c *Coordinator) viewFor(uuid string) ClusterView {
	...
	if err != nil {
		slog.Warn("clustersync: registry read failed building cluster view", "uuid", uuid, "err", err)

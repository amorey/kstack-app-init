...
// testSpec/testStatus stand in for any kind's shapes — watchListStream is generic over
...
// runWatchList pumps a beehive stream through watchListStream, folding each change to
// a frame. It returns the frame channel: beehive's streamFail is unexported, so a
// hand-built source cannot fail and these tests never read Err. Stream's own reporting
// is covered in stream_test.go, and end-to-end in graph/watch_failure_test.go.
func runWatchList(
	ctx context.Context,
	...
	return watchListStream(ctx, "Test", stream,
...
// reported complete.
func TestWatchListStreamBookmarksBetweenSnapshotAndDeltas(t *testing.T) {
...
// "not yet".
func TestWatchListStreamBookmarksAnEmptySnapshotImmediately(t *testing.T) {
	ctx, cancel := context.WithCancel(context.Background())
	defer cancel()
...
func TestWatchListStreamCollapsesDeletionPendingToDeleted(t *testing.T) {
	ctx, cancel := context.WithCancel(context.Background())
	defer cancel()
...
// deliver again.
func TestWatchListStreamEndsWhenTheSourceCloses(t *testing.T) {
	ctx, cancel := context.WithCancel(context.Background())
	defer cancel()
...
func TestWatchListStreamClosesOnContextCancel(t *testing.T) {
	ctx, cancel := context.WithCancel(context.Background())
	out := runWatchList(ctx, nil, make(chan beehive.ObjectChange[testSpec, testStatus]))
	require.Equal(t, frame{typ: domain.DeltaFrameBookmark}, recv(t, out))

...
// defaultRetention is what the production sidecar uses. Tests can
// override by calling startSync with a custom retention (not yet
...
func runJanitor(ctx context.Context, uuid string, writer *sql.DB, ret Retention) {
...
func sweep(ctx context.Context, uuid string, writer *sql.DB, ret Retention) {
	...
	if res, err := writer.ExecContext(ctx,
		...
		slog.Warn("janitor: events sweep failed", "uuid", uuid, "err", err)
	} else if n, _ := res.RowsAffected(); n > 0 {
		slog.Debug("janitor: trimmed events", "uuid", uuid, "rows", n)
		totalDeleted += n
	}
	...
	if res, err := writer.ExecContext(ctx,
		...
		slog.Warn("janitor: status_history sweep failed", "uuid", uuid, "err", err)
	} else if n, _ := res.RowsAffected(); n > 0 {
		slog.Debug("janitor: trimmed status_history", "uuid", uuid, "rows", n)
		totalDeleted += n
	}
	...
	if totalDeleted > 0 {
		if _, err := writer.ExecContext(ctx, `PRAGMA incremental_vacuum`); err != nil {
			slog.Warn("janitor: incremental_vacuum failed", "uuid", uuid, "err", err)
		}
	}
}
